...
def write_to_csv(poilist, citycode, classfield, coord):
    data_csv = {}
    lons, lats, names, addresss, pnames, citynames, business_areas, types, typecodes, ids, type_1s, type_2s, type_3s, type_4s,parents,childtypes = [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []
    ...
    for i in range(len(poilist)):
        ...
        childtype = poilist[i].get('childtype')
        if (coord == 2):
    ...
    data_csv['cityname'], data_csv['business_area'], data_csv['type'], data_csv['typecode'], data_csv['id'], data_csv[
        'type1'], data_csv['type2'], data_csv['type3'], data_csv['type4'],data_csv['parent'],data_csv['childtype'] = \
        lons, lats, names, addresss, pnames, citynames, business_areas, types, typecodes, ids, type_1s, type_2s, type_3s, type_4s,parents,childtypes
    ...
    file_name = 'poi-' + citycode + "-" + classfield + "-" + time.strftime("%m%d%H%M", time.localtime()) + ".csv"
